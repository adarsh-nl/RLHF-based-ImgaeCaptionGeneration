...
try:
    ...
    for img in batches[0]:
        ...
        for i in range(max_length):
            sequence = tokenizer.texts_to_sequences([in_text2])[0]
            sequence = pad_sequences([sequence], maxlen=max_length)
            preds2 = model.predict([photo, sequence], verbose=0)
            next_index2 = sample_pred(preds2, temperature=0.5)
            word2 = word_for_id(next_index2, tokenizer)
            if word2 is None:
                break
            in_text2 += ' ' + word2
            prob2 *= preds2[0][next_index2]
            if word2 == 'end':
                break

        # Gather feedback from user and store in preference dataset
        print("Caption 1:", in_text1)
        print("Caption 2:", in_text2)
        feedback1 = float(input("Rate the quality of caption 1 (0-1): "))
        feedback2 = float(input("Rate the quality of caption 2 (0-1): "))
        # Calculate probability of each caption being correct
        prob1 *= feedback1
        prob2 *= feedback2
        total_prob = prob1 + prob2
        preference_dataset.append((img, in_text1, in_text2, prob1/total_prob, prob2/total_prob))

except KeyboardInterrupt:
    print("Interrupted! Saving preference dataset...")
    columns = ["image", "caption1", "caption2", "feedback1", "feedback2"]
    with open("preference_dataset.csv", "w", newline="") as csvfile:
        ...
        for data in preference_dataset:
            ...
            feedback1 = data[3]
            feedback2 = data[4]
            writer.writerow([photo, caption1, caption2, feedback1, feedback2])
...
# Open a CSV file in write mode and write the column names to the first row
with open("preference_dataset.csv", "w", newline="") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(columns)
    ...
    for data in preference_dataset:
        ...
        caption2 = data[2]
        feedback1 = data[3]
        feedback2 = data[4]  # Calculate feedback for the second caption
        writer.writerow([photo, caption1, caption2, feedback1, feedback2])
...
Gather a dataset of image-caption pairs. This dataset will be used to train your model.

Fine-tune a pre-trained language model on your image-caption dataset using standard techniques, such as stochastic gradient descent or Adam optimization.

Generate captions for a subset of your images using the fine-tuned model.

Ask human evaluators to rate the quality of the generated captions on a scale of, say, 1 to 5.

Train a new model on your original image-caption dataset, but with the addition of a new "preference dataset" that consists of pairs of generated captions for each image, along with their human ratings.

Fine-tune the new model on the preference dataset, using a loss function that minimizes the difference between the predicted and actual human ratings.

Repeat steps 3-6 until the quality of the generated captions reaches a satisfactory level.
"""
...
preference_dataset = pd.read_csv("/content/drive/MyDrive/CV Project - Image caption generator/preference_dataset.csv")
...
def weighted_rating_loss(y_true, y_pred):
...
for img in os.listdir(dataset_images): # Number of RL iterations
    ...
    if RL_learning_loop < 10:
        ...
        image.show()
        photo = extract_features(image_path, xception_model)
        print("Shape of the features: ", photo.shape)
        # Get captions and ratings for this image
        captions = []
        ratings = [] 
        for i in range(1, 3):
            caption_col = 'caption' + str(i)
            feedback_col = 'feedback' + str(i)
            caption = preference_dataset.loc[preference_dataset['image'] == img, caption_col].iloc[0]
            print("Caption: ",caption)
            feedback = preference_dataset.loc[preference_dataset['image'] == img, feedback_col].iloc[0]
        ...
        print("Shape of rating's:", ratings.shape)
        print("Shape of captions:", captions.shape)
        # Compute losses and update weights for each caption
        for i, caption in enumerate(captions):
            sequence = tokenizer.texts_to_sequences([caption])[0]
            print("Sequence before padding: ",sequence)
            sequence = pad_sequences([sequence], maxlen=max_length)
            print("Sequence after padding: ",sequence)
            predicted_rating = q_table[len(sequence)-1][sequence[0][-1]]
            predicted_rating = np.ravel(predicted_rating) # add this line to reshape the predicted rating
            print("Predicted rating: ",predicted_rating)
            human_rating = ratings[i]
            print("Human Rating: ", human_rating)
            human_rating = np.array([human_rating]).reshape(1,)
            loss = (predicted_rating - human_rating) * human_rating
            loss = np.array([loss]).reshape(1,1)
            model.train_on_batch([photo, sequence], human_rating, sample_weight=loss)
        # Save updated model weights
        model.save('/content/drive/MyDrive/CV Project - Image caption generator/Models/model_rl.h5')
